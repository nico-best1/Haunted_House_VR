# Haunted House VR

**Haunted House VR** is an immersive virtual reality horror experience developed with **Unity**.  
Explore a mysterious abandoned mansion packed with secrets, scares, and an eerie atmosphere designed to fully immerse the player in a terrifying experience.

---

## Demo Video

[![Haunted House VR – Demo](https://img.youtube.com/vi/dfJRu-ihpig/maxresdefault.jpg)](https://youtu.be/dfJRu-ihpig)

---

## Main Features

- Fully immersive **virtual reality** experience  
- Detailed environments with dynamic lighting  
- 3D ambient sound  
- Interaction with physical objects through VR controllers  
- Visual and audio effects designed to create psychological tension  

---

## Technologies and Tools Used

- **Engine:** Unity, version 6000.2.9f1  
- **Programming Language:** C#  
- **VR SDK:** OpenXR  
- **Audio:** FMOD  

---

## Controls and Interaction

- **Movement:** Continuous using joystick  
- **Interaction:** Press trigger or grip button

## Assets

- Victorian Mansion (https://assetstore.unity.com/packages/3d/props/furniture/victorian-mansion-169718)
- Alchemist House (https://assetstore.unity.com/packages/3d/environments/alchemist-house-112442)
- Old classical chair (https://assetstore.unity.com/packages/3d/props/furniture/old-classical-chair-46865)
- Picture Frames (https://assetstore.unity.com/packages/3d/props/furniture/picture-frames-301169)
- AllSky Free - 10 Sky / Skybox Set (https://assetstore.unity.com/packages/2d/textures-materials/sky/allsky-free-10-sky-skybox-set-146014)
