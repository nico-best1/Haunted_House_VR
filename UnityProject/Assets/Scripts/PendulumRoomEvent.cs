...
public class PendulumRoomEvent : MonoBehaviour
{
    ...
    public AudioSource flickerAudio;  // Si a�n quieres un audio extra para el parpadeo
    ...
    private IEnumerator EventSequence()
    {
        ...
        float duration = 20f;
        StartCoroutine(FlickerLights()); // Llamamos a la nueva l�gica de parpadeo
    ...
    private IEnumerator MoveDrawers()
    {
        while (!eventFinished)
        {
            ...
            yield return new WaitForSeconds(Random.Range(1f, 2f));  // Pausa m�s larga para evitar saturaci�n
        }
    }
